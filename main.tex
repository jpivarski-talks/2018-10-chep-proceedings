...
\noindent can be replaced by the following array code.

{\small\begin{minted}{python}
pairs = events["jets"].cross(events["leptons"])  # cross-join particles per event
goodpairs = pairs[cut(pairs["0"], pairs["1"])]   # pairs is a table of "0" and "1"
candidates = pairs["0"] + pairs["1"]             # Lorentz vector + operator
best = candidates[quality(candidates).argmax()]  # get best indexes, then best objects
leptoquarks = best.flatten()                     # reduce structure to flat array
\end{minted}
}
